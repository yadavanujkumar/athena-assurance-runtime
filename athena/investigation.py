        ...
        if kind == "dependency_review":
            ...
            for ecosystem in sorted({d.ecosystem for d in deps}):
                ...
                for advisory in audit.findings:
                    ...
                    finding = Finding(
                        f"DEP-{ecosystem}-{name}".replace(" ", "-")[:64],
                        f"Dependency vulnerability: {name}",
                        "An installed open-source dependency auditor reported a vulnerability or advisory.",
                        Severity.HIGH,
                        0.9,
                        [audit.tool, name],
                        "Upgrade, replace or constrain the affected dependency and rerun the audit.",
                    )
            ...
            commands = self.validation.detect_commands(self.root)
            for command in commands:
                ...
                if result.available and not result.passed:
                    findings.append(Finding(
                        "VALIDATION-FAILED-" + str(abs(hash(command)))[:10],
                        "Project validation failed",
                        f"Detected validation command failed: {command}",
                        Severity.HIGH,
                        0.95,
                        [command, result.stderr[-1000:]],
                        "Investigate the failing test or validation command before accepting the project state.",
                    ))
        else:
