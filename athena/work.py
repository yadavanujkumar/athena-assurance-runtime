...
class WorkItem:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"id": self.id, "kind": self.kind, "reason": self.reason, "objective": self.objective, "priority": self.priority, "status": self.status, "attempts": self.attempts, "created_at": self.created_at, "updated_at": self.updated_at, "last_error": self.last_error, "depends_on": list(self.depends_on), "context_key": self.context_key}
...
class WorkQueue:
    ...
    def enqueue(self, *, kind: str, reason: str, priority: int, objective: str | None = None, depends_on: tuple[str, ...] = (), context_key: str = "") -> WorkItem:
        item_id = self.make_id(kind, reason, objective, context_key)
        now = utc_now()
        existing = self.get(item_id)
        if existing:
            if existing.status in {"failed", "cancelled"}:
                self.db.execute("UPDATE work_items SET status='queued', priority=?, updated_at=?, last_error=NULL, depends_on=? WHERE id=?", (max(priority, existing.priority), now, json.dumps(list(depends_on)), item_id))
                self.db.commit()
            ...
            if priority > existing.priority:
                ...
                return self.get(item_id)  # type: ignore[return-value]
            return existing
        self.db.execute("INSERT INTO work_items(id,kind,reason,objective,priority,status,attempts,created_at,updated_at,last_error,depends_on,context_key) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", (item_id, kind, reason, objective, priority, "queued", 0, now, now, None, json.dumps(list(depends_on)), context_key))
        self.db.commit()
        return self.get(item_id)  # type: ignore[return-value]
    ...
    def claim_next(self, max_attempts: int = 3) -> WorkItem | None:
        ...
        try:
            ...
            if selected is None:
                ...
                return None
            now = utc_now()
            self.db.execute("UPDATE work_items SET status='running', attempts=attempts+1, updated_at=? WHERE id=? AND status='queued'", (now, selected["id"]))
            self.db.commit()
    ...
    def recover_running(self) -> list[WorkItem]:
        ...
        if not rows:
            return []
        now = utc_now()
        self.db.execute("UPDATE work_items SET status='queued', last_error=?, updated_at=? WHERE status='running'", ("Recovered after runtime restart; previous worker was interrupted.", now))
        self.db.commit()
    ...
    def unblock_ready(self) -> list[WorkItem]:
        ...
        for row in rows:
            ...
            if dependencies and not all(self._dependency_satisfied(dep) for dep in dependencies):
                continue
            self.db.execute("UPDATE work_items SET status='queued', last_error=NULL, updated_at=? WHERE id=? AND status='blocked'", (utc_now(), row["id"]))
    ...
    def _row(row: sqlite3.Row) -> WorkItem:
        return WorkItem(id=row["id"], kind=row["kind"], reason=row["reason"], objective=row["objective"], priority=row["priority"], status=row["status"], attempts=row["attempts"], created_at=row["created_at"], updated_at=row["updated_at"], last_error=row["last_error"], depends_on=tuple(json.loads(row["depends_on"])), context_key=row["context_key"])
