...
class Memory:
    ...
    def _init(self) -> None:
        self.db.executescript("""
            ...
            CREATE TABLE IF NOT EXISTS facts (key TEXT PRIMARY KEY, value TEXT NOT NULL, updated_at TEXT NOT NULL);
    ...
    def search_events(self, term: str, limit: int = 20) -> list[dict]:
        pattern = f"%{term}%"
        rows = self.db.execute("SELECT * FROM events WHERE kind LIKE ? OR payload LIKE ? ORDER BY id DESC LIMIT ?", (pattern, pattern, limit))
    ...
    def add_finding(self, finding: Finding) -> None:
        ...
        self.db.execute("INSERT OR REPLACE INTO findings(id,payload,updated_at) VALUES(?,?,?)", (finding.id, json.dumps(payload, sort_keys=True), utc_now()))
        self.db.commit()
    ...
    def add_decision(self, decision: Decision) -> None:
        ...
        self.db.execute("INSERT OR REPLACE INTO decisions(id,payload,updated_at) VALUES(?,?,?)", (decision.id, json.dumps(payload, sort_keys=True), utc_now()))
        self.db.commit()
